...
func TestNewAccessPolicy(t *testing.T) {
	...
	p := accesspolicy.NewAccessPolicy(nil, nil, false, false)
	a.NotNil(p)
	a.Nil(p.Owner)
	a.Nil(p.Parent)
	...
	// with owner
	user, err := user.UserNew("testuser", "testuser@example.com", testReusableUserinfo)
	a.NoError(err)

	p = accesspolicy.NewAccessPolicy(user, nil, false, false)
	a.NotNil(p)
	...
	// with parent
	parent := accesspolicy.NewAccessPolicy(nil, nil, false, false)
	a.NoError(err)

	p = accesspolicy.NewAccessPolicy(nil, parent, false, false)
	a.NotNil(p)
	a.Nil(p.Owner)
	...
	// with inheritance
	p = accesspolicy.NewAccessPolicy(nil, nil, true, false)
	a.NotNil(p)
	a.Nil(p.Owner)
	a.Nil(p.Parent)
	...
	// with extension
	p = accesspolicy.NewAccessPolicy(nil, nil, false, true)
	a.NotNil(p)
	a.Nil(p.Owner)
	a.Nil(p.Parent)
	a.False(p.IsInherited)
...
func TestSetPublicRights(t *testing.T) {
	...
	assignor, err := user.UserNew("assignor", "testuser@example.com", testReusableUserinfo)
	a.NoError(err)

	testuser, err := user.UserNew("testuser", "testuser@example.com", testReusableUserinfo)
	a.NoError(err)

	wantedRights := accesspolicy.APView | accesspolicy.APChange

	// no parent, not inheriting and not extending
	p := accesspolicy.NewAccessPolicy(assignor, nil, false, false)
	...
	pWithInheritance := accesspolicy.NewAccessPolicy(assignor, p, true, false)
	...
	// no own rights
	pExtendedNoOwn := accesspolicy.NewAccessPolicy(assignor, p, false, true)
	a.NoError(err)
	a.Equal(wantedRights, pExtendedNoOwn.Parent.RightsRoster.Everyone)
	a.Equal(accesspolicy.APNoAccess, pExtendedNoOwn.RightsRoster.Everyone)
	...
	pExtendedWithOwn := accesspolicy.NewAccessPolicy(assignor, p, false, true)
	pExtendedWithOwn.SetPublicRights(assignor, accesspolicy.APMove)
	a.NoError(err)
	a.Equal(wantedRights, pExtendedWithOwn.Parent.RightsRoster.Everyone)
	a.Equal(accesspolicy.APMove, pExtendedWithOwn.RightsRoster.Everyone)
	a.True(pExtendedWithOwn.HasRights(testuser, wantedRights|accesspolicy.APMove))
}
...
func TestSetGroupRights(t *testing.T) {
	...
	group1, err := groupManager.Create(group.GKGroup, "test_group_1", "test group 1", nil)
	a.NoError(err)

	err = group1.AddMember(testuser)
	a.NoError(err)

	group2, err := groupManager.Create(group.GKGroup, "test_group_2", "test group 2", nil)
	a.NoError(err)
	...
	// NOTE: added core.APMove to own rights
	pExtendedWithOwn := accesspolicy.NewAccessPolicy(assignor, p, false, true)
	pExtendedWithOwn.SetGroupRights(assignor, group1, accesspolicy.APMove)
	a.NoError(err)
...
func TestSetRoleRights(t *testing.T) {
	...
	// adding the user to 2 groups but setting rights to only one
	role1, err := gc.Create(group.GKRole, "test_role_1", "test role 1", nil)
	a.NoError(err)

	err = role1.AddMember(testuser)
	a.NoError(err)

	role2, err := gc.Create(group.GKRole, "test_role_2", "test role 2", nil)
	a.NoError(err)
	...
	// NOTE: added core.APMove to own rights
	pExtendedWithOwn := accesspolicy.NewAccessPolicy(assignor, p, false, true)
	pExtendedWithOwn.SetRoleRights(assignor, role1, accesspolicy.APMove)
	a.NoError(err)
...
func TestSetUserRights(t *testing.T) {
	...
	a.Equal(accesspolicy.APNoAccess, pExtendedNoOwn.RightsRoster.User[testuser.ID])
	a.True(pExtendedNoOwn.HasRights(testuser, wantedRights))
	...
	// NOTE: added core.APMove to own rights
	pExtendedWithOwn := accesspolicy.NewAccessPolicy(assignor, p, false, true)
	pExtendedWithOwn.SetUserRights(assignor, testuser, accesspolicy.APMove)
	a.NoError(err)
	a.Equal(wantedRights, pExtendedWithOwn.Parent.RightsRoster.User[testuser.ID])
	a.Equal(accesspolicy.APMove, pExtendedWithOwn.RightsRoster.User[testuser.ID])
	a.True(pExtendedWithOwn.HasRights(testuser, wantedRights|accesspolicy.APMove))
}
...
func TestIsOwner(t *testing.T) {
	...
	p := accesspolicy.NewAccessPolicy(testuser, nil, false, false)
	err = p.SetUserRights(testuser, testuser, accesspolicy.APView)
	a.NoError(err)
	a.Equal(accesspolicy.APView, p.RightsRoster.User[testuser.ID])
	a.Equal(accesspolicy.APNoAccess, p.RightsRoster.User[testuser2.ID])
	a.True(p.HasRights(testuser, accesspolicy.APFullAccess))
	a.False(p.HasRights(testuser2, accesspolicy.APView))
	a.False(p.HasRights(testuser2, accesspolicy.APFullAccess))
...
func TestAccessPolicyClone(t *testing.T) {
	...
	p := accesspolicy.NewAccessPolicy(owner, nil, false, false)
	err = p.SetUserRights(owner, owner, accesspolicy.APView)
	a.NoError(err)
	a.Equal(accesspolicy.APView, p.RightsRoster.User[owner.ID])
	a.Equal(accesspolicy.APNoAccess, p.RightsRoster.User[testuser.ID])
	a.True(p.HasRights(owner, accesspolicy.APFullAccess))
	a.False(p.HasRights(testuser, accesspolicy.APView))
	a.False(p.HasRights(testuser, accesspolicy.APFullAccess))
...
func TestAccessPolicyUnsetRights(t *testing.T) {
	...
	role, err := groupContainer.Create(group.GKRole, "test_role_group", "Test Role Group", nil)
	a.NoError(err)

	group, err := groupContainer.Create(group.GKGroup, "test_group", "Test Group", nil)
	a.NoError(err)
	...
	//---------------------------------------------------------------------------
	// setting
	ap := accesspolicy.NewAccessPolicy(assignor, nil, false, false)
...
func TestHasGroupRights(t *testing.T) {
	...
	// adding the user to 2 groups but setting rights to only one
	group1, err := groupContainer.Create(group.GKGroup, "test_group_1", "test group 1", nil)
	a.NoError(err)

	group2, err := groupContainer.Create(group.GKGroup, "test_group_2", "test group 2", group1)
	a.NoError(err)

	group3, err := groupContainer.Create(group.GKGroup, "test_group_3", "test group 3", group2)
	a.NoError(err)
