...
const (
	...
	APView     = AccessRight(1 << uint32(iota-1))
	APCreate
	APChange
	APDelete
	APCopy
...
func AccessPolicyDictionary() map[uint32]string {
	...
	for bit := AccessRight(1 << 31); bit > 0; bit >>= 1 {
...
type AccessPolicy struct {
	...
	Checksum    uint32      `db:"checksum" json:"checksum"`
...
func NewAccessPolicy(ownerID, parentID uint32, name string, objectType string, objectID uint32, isInherited, isExtended bool) (ap AccessPolicy, err error) {
...
func (ap *AccessPolicy) calculateChecksum() uint32 {
...
// Key returns a uint32 key hash to be used as a map/cache key
func (ap *AccessPolicy) Key(rehash bool) uint32 {
...
// TODO: move to the Manager
func (ap *AccessPolicy) SetRoleRights(ctx context.Context, assignorID uint32, roleID uint32, rights AccessRight) error {
	if assignorID == 0 {
...
// TODO: move to the Manager
func (ap *AccessPolicy) SetGroupRights(ctx context.Context, assignorID uint32, groupID uint32, rights AccessRight) (err error) {
	if assignorID == 0 {
		return ErrZeroAssignorID
	}
