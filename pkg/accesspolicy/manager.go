...
func (m *AccessPolicyManager) Create(ctx context.Context, ownerID, parentID uint32, name string, objectType string, objectID uint32, isInherited, isExtended bool) (ap AccessPolicy, err error) {
...
// TODO: add cache
func (m *AccessPolicyManager) PolicyByObjectTypeAndID(ctx context.Context, objectType string, objectID uint32) (ap AccessPolicy, err error) {
	// attempting to obtain policy from the store
...
func (m *AccessPolicyManager) UnsetRights(ctx context.Context, ap AccessPolicy, assignorID uint32, subject interface{}) (err error) {
	if err = ap.Validate(); err != nil {
		return err
	}
