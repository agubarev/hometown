...
func TestMiddlewareAuthenticatorUserSuspended(t *testing.T) {
	...
	// suspending user
	a.NoError(userManager.SuspendUser(ctx, testuser.ID, "testing middleware", time.Now().Add(24*time.Hour)))
...
func TestMiddlewareAuthenticatorInvalidAccessToken(t *testing.T) {
	...
	privateKey, err := authenticator.PrivateKey()
	a.NoError(err)
	a.NotNil(privateKey)

	// initializing new access token
	wrongAccessToken, err := auth.NewAccessToken(
		privateKey,
		uuid.New(),
		auth.UserIdentity(uuid.New()),
		time.Now().Add(time.Hour),
	)
	a.NoError(err)
	a.NotEmpty(wrongAccessToken)

	// setting up test request
	req = req.WithContext(ctx)
	req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", wrongAccessToken))
	req.Header.Set("Content-Type", "application/json")
	rr := httptest.NewRecorder()
