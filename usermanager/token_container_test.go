...
func TestTokenContainerNewToken(t *testing.T) {
	...
	tok, err := usermanager.NewToken(usermanager.TkUserEmailConfirmation, id, 10*time.Second, 1)
	a.NoError(err)
	a.NotNil(tok)
	a.Equal(usermanager.TkUserEmailConfirmation, tok.Kind)
...
func TestTokenContainerCreateGetAndDelete(t *testing.T) {
	...
	// creating new token
	tok, err := c.Create(usermanager.TkUserEmailConfirmation, id, 10*time.Second, 1)
	a.NoError(err)
	a.NotNil(tok)
	a.Equal(usermanager.TkUserEmailConfirmation, tok.Kind)
	a.True(len(tok.Payload) > 0)
...
func TestTokenContainerCheckin(t *testing.T) {
	...
	flagSwitch := false
	err = c.AddCallback(tok.Kind, callbackID, func(ctx context.Context, t *usermanager.Token) error {
		flagSwitch = true
		return nil
	})
	a.NoError(err)
	...
	tok, err = c.Create(usermanager.TkUserEmailConfirmation, payload, 10*time.Second, 2)
	a.NoError(err)
	a.NotNil(tok)
	a.Equal(usermanager.TkUserEmailConfirmation, tok.Kind)
	a.True(len(tok.Payload) > 0)
	...
	err = c.Checkin(tok.Token)
	a.EqualError(usermanager.ErrTokenCallbackNotFound, err.Error())
...
func TestTokenContainerAddCallback(t *testing.T) {
	...
	// registering a proper callback
	err = c.AddCallback(tok.Kind, validID, func(ctx context.Context, t *usermanager.Token) error {
		return nil
	})
	a.NoError(err)
	...
	a.EqualError(usermanager.ErrTokenDuplicateCallbackID, err.Error())
	...
	a.Error(usermanager.ErrTokenCallbackNotFound, err.Error())
	a.Nil(cb)
}
...
func TestTokenContainerCleanup(t *testing.T) {
	...
	c.Create(usermanager.TkUserEmailConfirmation, util.NewULID(), 4*time.Second, 1)
	c.Create(usermanager.TkUserEmailConfirmation, util.NewULID(), 4*time.Second, 1)
	c.Create(usermanager.TkUserEmailConfirmation, util.NewULID(), 4*time.Second, 1)
	c.Create(usermanager.TkUserEmailConfirmation, util.NewULID(), 6*time.Second, 1)
	c.Create(usermanager.TkUserEmailConfirmation, util.NewULID(), 7*time.Second, 1)
	c.Create(usermanager.TkUserEmailConfirmation, util.NewULID(), 8*time.Second, 1)
	c.Create(usermanager.TkUserEmailConfirmation, util.NewULID(), 16*time.Second, 1)

	a.Len(c.List(usermanager.TkAllTokens), 7)
